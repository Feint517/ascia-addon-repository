...
Device auto-discovery, onboarding, naming, and area assignment. Format follows
...
- **The `room` widget type was never migrated, and it took the whole home page
  with it.** Phase 4b renamed two widget types together; `_V2` migrated only
  `lights-room`. Layouts holding a `room` tile kept a type the frontend no longer
  has — and because the grid indexed its catalog with that string and read a field
  off `undefined`, the throw blanked *every* widget, not just the stale one. A
  dashboard of twenty tiles rendered as nothing, found on the appliance by its
  owner rather than by any test here.

  `_V3` renames `"type":"room"` → `"type":"area"` in stored layouts, anchored on
  the `"type":` key so a label someone typed is left alone — the old name here is
  an ordinary English word, unlike `lights-room`.

  The renderer stopped trusting the lookup too (frontend `widgetMeta`), so an
  unknown type now costs one tile instead of the page. Both were needed.

- A new test reads the frontend's `WidgetType` union directly and asserts every
  type this add-on can write — the seed and each migration's replacement — is one
  the renderer has an entry for. A hand-kept list here would have drifted the same
  way the two sides of the rename did.
...
- **`zone` → `area`.** `GET/POST /devices/zones` → `/devices/areas`;
  `POST /devices/{device_id}/zone` → `/area`. The `Zone`, `ZoneRequest` and
  `CreateZoneRequest` models are `Area*`; operation ids are `listAreas`,
  `createArea`, `assignDeviceArea`. ASCIA and Home Assistant now use one word for
  one thing, so `Area.area_id` reads as itself rather than as a translation.
- **`Device.area` → `Device.area_name`.** It holds the area's *display name*,
  and sat directly beside `area_id`, which holds the identifier — the one
  genuinely misleading field in this add-on's model.
- `DiscoveredDevice.suggested_zone` → `suggested_area_id`; `suggest_zone()` →
  `suggest_area()`. The `unzoned` wording becomes **`unassigned`**, which is what
  it always meant.
- `can_control(role, user_zones, entity_zone, domain)` →
  `(role, user_areas, entity_area, domain)`. Names only; the permission matrix
  is unchanged and `test_control.py` proves it.

Sonos still advertises itself over SSDP as
`urn:schemas-upnp-org:device:ZonePlayer:1`. That string is UPnP's, not ours, and
is deliberately untouched — renaming it would stop discovery matching real
hardware.
